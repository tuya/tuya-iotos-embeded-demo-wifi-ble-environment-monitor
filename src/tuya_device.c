...
STATIC VOID wifi_state_led_reminder(IN CONST GW_WIFI_NW_STAT_E cur_stat)
{
    switch (cur_stat)
    {
        ...
        case STAT_UNPROVISION: //ez 配网模式，等待连接
        ...
        case STAT_STA_DISC: //ez,ap 正在连接中
...
STATIC VOID wifi_config_init(VOID)
{
    ...
    key_def.seq_key_detect_time = WIFI_KEY_SEQ_PRESS_MS;    //多少ms内再次按下算连按
...
#ifdef ENABLE_UART_2_PRINT
//日志打印串口切换到 uart2 
#define TY_UART TY_UART2
...
{
#ifdef ENABLE_UART_2_PRINT
	/* 设置 uart2 为日志打印口，波特率设置为9600*/
    ty_set_log_port(LOG_PORT2);
    ty_uart_init_mk(TY_UART2, TYU_RATE_9600, TYWL_8B, TYP_NONE, TYS_STOPBIT1);
#endif
 ...
 * @Others: none
 */
STATIC VOID dev_dp_query_cb(IN CONST TY_DP_QUERY_S *dp_qry)   //进入面板中，查询回调 
{
